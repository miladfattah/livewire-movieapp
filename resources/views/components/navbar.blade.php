<div class="w-full text-gray-700 bg-white dark:text-gray-200 dark:bg-gray-800">
    <div x-data="{ open: false }"
        ...
        <nav :class="{'flex': open, 'hidden': !open}"
            ...
            <a class="{{request()->routeIs('movie.index') ? 'bg-gray-600' : ''}} px-4 py-2 mt-2 text-sm font-semibold rounded-lg dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                href="{{route('movie.index')}}">Movies</a>
            <a class="{{request()->routeIs('serie.index') ? 'bg-gray-600' : ''}}px-4 py-2 mt-2 text-sm font-semibold rounded-lg dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                href="{{route('serie.index')}}">Series</a>
            <a class="{{request()->routeIs('cast.index') ? 'bg-gray-600' : ''}} px-4 py-2 mt-2 text-sm font-semibold rounded-lg dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                href="{{route('cast.index')}}">Casts</a>
            <div @click.away="open = false" class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    class="absolute z-50 right-0  w-full mt-2 origin-top-right rounded-md shadow-lg md:w-96">
                    <div class="px-2 py-2 bg-white rounded-mdshadow dark:bg-gray-800 flex flex-wrap">
                        @foreach (App\Models\Genre::all() as $genre)
                            <a class="{{request()->routeIs('genre.*') ? 'bg-gray-600' : ''}} text-sm p-2 m-2 font-semibold bg-transparent rounded-lg dark:bg-transparent dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
